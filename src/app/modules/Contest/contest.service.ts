...
const createRecurringContest = async (creatorId: string, body: contestData, banner: Express.Multer.File) => {
    ...
    try {
        ...
        throw new ApiError(httpstatus.BAD_REQUEST, " recurring Contest creation failed", err.message)
    }
}
...
const uploadPhotoToContest = async (contestId: string, userId: string, photoIds: string[], file: Express.Multer.File) => {
    ...
    const images = await prisma.$transaction(async tx => {
        ...
        contestParticipantId = contestParticipant?.id ?? contestParticipantId
    ...
    if (shouldCreateWatcher && contestParticipantId) {
        try {
            await createExposureWatcher(contestParticipantId)
        } catch (err) {
            console.log("exposure watcher creation failed after transaction commit for participant", contestParticipantId, err)
        }
    }
